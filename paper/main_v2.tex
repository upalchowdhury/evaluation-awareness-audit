...
\IfFileExists{numbers_v2.tex}{\input{numbers_v2.tex}}{}
...
What I found is that the three instruments disagree with each other, and that
...
could, in principle, behave better under test than in use. Recent work on
open models established the first half of that worry: a linear probe on the
residual stream separates evaluation-framed from deployment-framed prompts
\citep{nguyen2025probing}. The same line of work raised a doubt about its own
evidence, that such a probe may be reading prompt format rather than context
\citep{devbunova2026format}. Whether the recognition reaches what the model
says, and whether either reaches what it does, is what I set out to
measure.
...
\resizebox{\linewidth}{!}{%
\IfFileExists{tables/landscape_v2a.tex}{\input{tables/landscape_v2a.tex}}{\todo{run make\_paper\_assets\_v2}}}
...
\setlength{\tabcolsep}{4pt}
\resizebox{\linewidth}{!}{%
\IfFileExists{tables/landscape_v2b.tex}{\input{tables/landscape_v2b.tex}}{\todo{run make\_paper\_assets\_v2}}}
...
\resizebox{\linewidth}{!}{%
\IfFileExists{tables/register_v2.tex}{\input{tables/register_v2.tex}}{\todo{run make\_paper\_assets\_v2}}}
...
exactly the variable under study. Nothing told me to make it, and I do not
claim anyone else has; but an unrelated prompt is an easy thing to reach for
when a control is wanted, so I think the trap is worth naming.
...
\begin{table}[t]\centering\footnotesize
\IfFileExists{tables/position_v2.tex}{\input{tables/position_v2.tex}}{\todo{run make\_paper\_assets\_v2}}
...
questions that give the models no reason to sandbag, and I claim nothing
beyond that.
...
This reference list is deliberately short: it names the work this paper
directly builds on or corrects, not the wider literature. Evaluation
awareness is linearly decodable from the residual stream of open models
\citep{nguyen2025probing}, and probe evidence for it has been challenged as
format sensitivity rather than context \citep{devbunova2026format};
Section~\ref{sec:reading} reproduces both results and comes down on the side
of the challenge. The control contrast I first used, and its repair in
Section~\ref{sec:patch}, is an instance of the bad-control problem in causal
inference \citep{cinelli2022crash}. Steering vectors that move a readout in
the direction opposite to the intended one have been reported before
\citep{torop2026inverted}; Section~\ref{sec:steer} adds a case measured with
a readout the model demonstrably answers.
...
\textbf{Goal.} My hypothesis is that a model can show
...
\textbf{Code review.} I read the code that builds the belief prompt,
...
\textbf{Prompts inspected.} I inspected a sample of prompt pairs and
...
\textbf{Numbers recomputed.} I used a separate script, which shares
